...
*/
static Hash *hashcreate (unsigned int n_hash)
{
 Hash *t = new (Hash);
 if (t == NULL)
 {
 ...
 nhash(t) = n_hash;
 markarray(t) = 0;
 nodelist(t) = newvector (n_hash, Node*);
 if (nodelist(t) == NULL)
 {
  lua_error ("not enough memory");
  return NULL;
...
*/
Hash *lua_createarray (int n_hash)
{
 ...
 new_list->array = hashcreate(n_hash);
